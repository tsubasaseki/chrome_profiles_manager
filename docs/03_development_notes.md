...
アイコンは `Get-ProfileIconImage` で 48px x 48px の `Bitmap` にリサイズして一覧へ渡します。
...
色選択は `DataGridViewComboBoxColumn` と、選択行へ直接色を設定するツールバー上の10色ボタンの両方から行えます。

色列の編集中に表示される `ComboBox` は `Set-ProfileColorComboBoxOwnerDraw` でオーナードロー化し、候補ごとに背景色、色見本、表示名を描画します。

色付きプルダウン候補の表示情報は `Get-ProfileColorComboItemVisual` で生成します。

更新日時の表示文字列は `Format-LastWriteTimeWithAge` が生成します。形式は `yyyy-MM-dd HH:mm:ss（n日前）` です。
...
ZIPバックアップ作成は、UIスレッドではなく Runspace 上で実行します。

処理状況は `New-BackupProgressState` で作成した同期ハッシュテーブルへ書き込み、UI側の WinForms `Timer` が定期的に読み取ります。

ファイル数の確認中は進捗バーを `Marquee` 表示にし、総ファイル数が分かった後は `Get-BackupProgressPercent` による百分率を表示します。

推定残り時間は `Get-BackupEstimatedRemainingText` が、ZIP追加開始時刻、処理済み件数、総ファイル数から算出します。

進捗ラベルの文字列は `Get-BackupProgressLabelText` が生成します。ラベルには個別ファイル名や `ZIPへ追加中` の詳細文言を入れず、全体進捗と推定残り時間だけを表示します。

個別ファイル名は `Write-UiLog` の `DEBUG` ログとして記録します。

バックグラウンド処理からGUI部品を直接操作しないことで、ZIP保存中の画面固まりを避けています。

ZIP作成完了後は `Select-ZipListItem` で完成ZIPを一覧上で選択し、ユーザーが希望した場合は既存の `Inspect-SelectedZip` で内容を確認します。
...
HTMLレポートの表は `.table-wrap` で横スクロール可能にし、`colgroup` と最小幅を使って列幅を安定させます。
...
- バックグラウンド処理中の詳細ログ不足
- ZIPバックアップ作成中にGUI全体が固まる問題
- HTMLレポートの列幅指定により、表が大きく崩れる問題
...
- ZIPバックアップ作成は非同期化済みですが、キャンセル機能はありません。将来的にはキャンセルボタンを追加できます。
